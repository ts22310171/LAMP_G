...
class cmain_node extends cnode
{
	...
	*/
	...
	function readdata()
	{
		...
		$obj = new cclient();
		$from = ($page - 1) * $limit;
		$client_rows = $obj->get_all(false, $from, $limit);
	}
	//--------------------------------------------------------------------------------------
	/*!
	@brief	削除
	...
	*/
	...
	function deljob()
	{
		if (isset($_POST['param']) && $_POST['param'] > 0) {
			...
			$change_obj->delete_core(false, 'clients', $where, $wherearr, false);
		}
	}
	//--------------------------------------------------------------------------------------
	/*!
	...
	*/
	...
	/*!
	@brief	クライアントのリストを得る
	@return	クライアントリストの文字列
	*/
	//--------------------------------------------------------------------------------------
	public function get_client_rows()
	{
		global $client_rows;
		global $page;
		$retstr = '';
		...
		if (count($client_rows) > 0) {
			foreach ($client_rows as $key => $value) {
				$javamsg =  '【' . $value['name'] . '】';
				$str = <<<END_BLOCK
<tr>
...
<a href="client_detail.php?mid={$value['id']}{$urlparam}">{$value['name']}</a>
</td>
...
END_BLOCK;
			...
			$retstr = <<<END_BLOCK
<tr><td colspan="3" class="nobottom">クライアントが見つかりません</td></tr>
END_BLOCK;
		}
		return $retstr;
	}
